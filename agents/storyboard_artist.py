...
- IMPORTANT: When describing people, NEVER mention body shape, physique, physical attributes, or form-fitting clothing. Use only safe descriptors like hair color, hair style, age range, and loose-fitting clothing colors. For example, use "wearing a blue shirt" instead of "wearing a form-fitting blue shirt", and "a person" instead of "a toned person".
- Avoid descriptions that focus on physical appearance or could be interpreted as suggestive. Keep all character descriptions family-friendly and conservative.
...
- **QUALITY ENHANCEMENTS**:
  - Create vivid, detailed visual descriptions that paint a clear picture for the viewer.
  - Include specific lighting details (e.g., "golden hour sunlight", "soft indoor lighting", "dramatic shadows").
  - Describe colors and textures to enhance visual richness (e.g., "vibrant green grass", "rough wooden texture", "glossy water surface").
  - Incorporate environmental details that support the mood and story (e.g., "wind blowing through trees", "gentle waves crashing", "distant city lights").
  - Use dynamic camera movements when appropriate to create visual interest (e.g., "slow dolly in", "gentle pan", "smooth tracking shot").
  - Ensure emotional continuity - shots should flow naturally and support the emotional arc of the scene.
  - Balance variety with coherence - shots should be visually diverse but still feel like part of the same scene.
"""
...
- **QUALITY ENHANCEMENTS**:
  - Include specific lighting details in frame descriptions (e.g., "warm afternoon light casting long shadows", "cool blue moonlight", "harsh fluorescent lighting").
  - Describe color palette and mood (e.g., "desaturated colors for melancholic mood", "vibrant colors for joyful scene", "high contrast for dramatic effect").
  - Add texture and material details (e.g., "rough stone wall", "smooth glass surface", "soft fabric drapes").
  - Incorporate atmospheric elements (e.g., "light fog", "dust particles in sunlight", "rain droplets on window").
  - Ensure visual continuity between frames - elements should move logically and consistently.
  - Use dynamic motion descriptions that create visual interest (e.g., "smooth crane shot rising", "handheld shake for intensity", "slow zoom for emphasis").
  - Consider depth of field and focus changes where appropriate (e.g., "shallow depth of field isolating subject", "rack focus from foreground to background").
"""
